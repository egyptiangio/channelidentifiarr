...
# Logo map cache (ID to URL mapping for channel display)
logos_map_cache = {
    'map': {},  # logo_id -> logo_url mapping
    'expires_at': None,
    'session_key': None
}
...
@app.route('/api/lineups/search-by-zip', methods=['GET'])
def search_lineups_by_zip():
    """Search for lineups by zip code"""
    try:
        postal_code = request.args.get('zip', '').strip()
        country = request.args.get('country', 'USA').upper()
        ...
        # Get all lineups for this zip code
        cursor.execute("""
            SELECT DISTINCT
                l.lineup_id,
                l.name,
        ...
        logger.error(f"Lineup search error: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/lineups/<lineup_id>/preview', methods=['GET'])
def preview_lineup(lineup_id):
    """Get preview of lineup with channel counts and sample channels"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        # Get lineup info
        cursor.execute("""
            SELECT lineup_id, name, type, device, mso_name
            FROM lineups
            WHERE lineup_id = ?
        """, (lineup_id,))
        ...
        if not lineup_row:
            return jsonify({'error': 'Lineup not found'}), 404

        lineup = dict_from_row(lineup_row)

        # Get quality breakdown
        cursor.execute("""
            SELECT
                COALESCE(video_type, 'UNKNOWN') as quality,
                COUNT(*) as count
        ...
        cursor.execute("""
            SELECT COUNT(*) FROM station_lineups WHERE lineup_id = ?
        """, (lineup_id,))
        ...
        cursor.execute("""
            ...
            JOIN station_lineups sl ON lc.station_id = sl.station_id AND lc.lineup_id = sl.lineup_id
            WHERE lc.lineup_id = ?
            ORDER BY CAST(lc.channel_number AS REAL)
        """, (lineup_id,))
        ...
        logger.error(f"Lineup preview error: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/lineups/<lineup_id>/import', methods=['POST'])
def import_lineup(lineup_id):
    ...
    try:
        ...
        # Data usage options
        use_logos = data.get('useLogos', True)
        use_call_sign = data.get('useCallSign', True)
        use_station_id = data.get('useStationId', True)
        ...
        include_hd = data.get('includeHd', True)
        include_uhd = data.get('includeUhd', True)
        include_unknown = data.get('includeUnknown', True)

        if not all([dispatcharr_url, dispatcharr_username, dispatcharr_password]):
            return jsonify({'error': 'Dispatcharr credentials required'}), 400

        # Validate conflict handling
        if number_conflict not in ['skip', 'overwrite']:
            return jsonify({'error': 'Invalid number conflict handling option'}), 400
        if station_conflict not in ['skip', 'move', 'create']:
            return jsonify({'error': 'Invalid station conflict handling option'}), 400

        # Get channels from lineup with quality filtering
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        quality_conditions = []
        if include_sd:
            quality_conditions.append("sl.video_type = 'SDTV'")
        if include_hd:
            quality_conditions.append("sl.video_type = 'HDTV'")
        if include_uhd:
            quality_conditions.append("sl.video_type = 'UHDTV'")
        if include_unknown:
            quality_conditions.append("(sl.video_type IS NULL OR sl.video_type = '')")

        if not quality_conditions:
            return jsonify({'error': 'At least one quality type must be selected'}), 400

        quality_filter_sql = "(" + " OR ".join(quality_conditions) + ")"
        ...
        cursor.execute(f"""
            ...
            FROM lineup_channels lc
            JOIN station_lineups sl ON lc.station_id = sl.station_id AND lc.lineup_id = sl.lineup_id
            JOIN stations s ON lc.station_id = s.station_id
            WHERE lc.lineup_id = ? AND {quality_filter_sql}
            ORDER BY CAST(lc.channel_number AS REAL)
        """, (lineup_id,))
        ...
        total_channels = len(channels_to_import)

        if total_channels == 0:
            return jsonify({'error': 'No channels match the selected quality filters'}), 400

        # Get existing Dispatcharr channels for duplicate detection
        existing_channels_result, error = dispatcharr_api_request(
        ...
        for ch in existing_channels:
            ...
            if ch_num:
                ...
                existing_by_number[ch_num].append(ch)

            station_id = ch.get('tvc_guide_stationid')
            if station_id:
                existing_by_station_id[station_id] = ch
        ...
        def generate():
            ...
            for idx, channel in enumerate(channels_to_import):
                try:
                    channel_num = channel['final_channel_number']
                    ...
                    if station_conflict == 'skip':
                        ...
                        if existing_with_station:
                            ...
                            _, update_error = dispatcharr_api_request(
                                dispatcharr_url, dispatcharr_username, dispatcharr_password,
                                'PATCH', f'/api/channels/channels/{old_channel_id}/',
                    ...
                    # Create new channel (for 'create' mode or if no duplicate found)
                    create_data = {
                        'name': channel_name,
                        'channel_number': channel_num,
                        'tvg_id': channel.get('call_sign', ''),
                    ...
                    if use_logos and channel.get('logo_uri'):
                        ...
                        logo_result, logo_error = dispatcharr_api_request(
                            dispatcharr_url, dispatcharr_username, dispatcharr_password,
                            'POST', '/api/channels/logos/', logo_data
                        )
                        if logo_result and logo_result.get('id'):
                            create_data['logo_id'] = logo_result['id']

                    # Create the channel
                    _, create_error = dispatcharr_api_request(
                        dispatcharr_url, dispatcharr_username, dispatcharr_password,
                        'POST', '/api/channels/channels/', create_data
                    )
                    ...
                    logger.error(f"Error importing channel {channel.get('station_name')}: {e}")
                    channels_failed += 1
...
def invalidate_logos_map_cache():
    """Invalidate the logos map cache (call when logos are created/updated/deleted)"""
    global logos_map_cache, logo_cache
    logos_map_cache['map'] = {}
    logos_map_cache['expires_at'] = None
    logos_map_cache['session_key'] = None
    # Also invalidate the URL cache to keep them in sync
    logo_cache['urls'] = {}
    logo_cache['expires_at'] = None
    logo_cache['session_key'] = None
    logger.info("Logo caches invalidated")
...
def find_or_create_logo(url, username, password, logo_url, logo_name, station_name=None):
    ...
    try:
        ...
        if logo_result and logo_result.get('id'):
            ...
            # Invalidate cache since a new logo was created
            invalidate_logos_map_cache()
...
def get_dispatcharr_channels():
    ...
    try:
        ...
        # Use cached logos map if available and not expired
        session_key = f"{url}:{username}"
        current_time = time.time()
        cache_valid = (
            logos_map_cache['session_key'] == session_key and
            logos_map_cache['expires_at'] and
            logos_map_cache['expires_at'] > current_time
        )

        if cache_valid:
            # Check if we have all needed logo IDs in cache
            missing_logo_ids = logo_ids - set(logos_map_cache['map'].keys())
            if missing_logo_ids:
                logger.info(f"Cache hit, but fetching {len(missing_logo_ids)} new logos")
                # Fetch only missing logos
                for logo_id in missing_logo_ids:
                    logo_data, _ = dispatcharr_api_request(url, username, password, 'GET', f'/api/channels/logos/{logo_id}/')
                    if logo_data:
                        logos_map_cache['map'][logo_id] = logo_data.get('url', '')
            else:
                logger.info(f"Using cached logos map ({len(logo_ids)} logos)")

            logos_map = logos_map_cache['map']
        else:
            # Cache miss or expired - rebuild entire map
            logger.info(f"Building fresh logos map for {len(logo_ids)} logos")
            logos_map = {}
            for logo_id in logo_ids:
                logo_data, _ = dispatcharr_api_request(url, username, password, 'GET', f'/api/channels/logos/{logo_id}/')
                if logo_data:
            ...
            # Update cache
            logos_map_cache['map'] = logos_map
            logos_map_cache['session_key'] = session_key
            logos_map_cache['expires_at'] = current_time + 300  # 5 minutes
...
def delete_emby_logos():
    """Delete selected channel logo types from Emby with SSE progress updates"""
    try:
        data = request.json
        url = data.get('url', '').rstrip('/')
        username = data.get('username')
        ...
        # Get logo types to delete (default to all if not specified for backward compatibility)
        logo_types_to_delete = data.get('logoTypes', ['Primary', 'LogoLight', 'LogoLightColor'])

        # Validate logo types
        valid_logo_types = ['Primary', 'LogoLight', 'LogoLightColor']
        logo_types_to_delete = [lt for lt in logo_types_to_delete if lt in valid_logo_types]

        if not logo_types_to_delete:
            return jsonify({'error': 'No valid logo types specified'}), 400
        ...
        def generate():
            ...
            for channel in channels:
                ...
                for logo_type in logo_types_to_delete:
